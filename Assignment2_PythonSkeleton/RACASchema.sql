...
drop function IF EXISTS CheckLoginCredentials(p_username VARCHAR(20), p_password VARCHAR(40));
drop function IF EXISTS findCarsByEmployee(p_userName VARCHAR(20));


-- Create function CheckLoginCredentials
CREATE OR REPLACE FUNCTION CheckLoginCredentials(p_username VARCHAR(20), p_password VARCHAR(40))
RETURNS TABLE(
	userName		VARCHAR(20),
	password		VARCHAR(40),
...
BEGIN
    ...
    SELECT *
    FROM Employee
    WHERE Employee.userName = p_username AND Employee.password = p_password;
END;
$$;

-- Create function findCarsByEmployee
CREATE OR REPLACE FUNCTION findCarsByEmployee(p_userName VARCHAR(20))
RETURNS TABLE(
    carID INTEGER,
    carinfo TEXT,
...
BEGIN
    ...
    SELECT 
        Car.carID, 
        Car.make || ' ' || Car.model as carinfo, 
        Status.statusName, 
        COALESCE(CarType.carTypeName, '') || '   ' || COALESCE(CarWheel.carWheelName, '') as typewheel, 
        Car.purchaseDate, 
        Employee.firstName || ' ' || Employee.lastName as name, 
        COALESCE(Car.description, '') as description
    FROM 
        Car 
    LEFT JOIN 
        Employee ON Car.managedBy = Employee.userName 
    LEFT JOIN 
        CarWheel ON Car.carWheelID = CarWheel.carWheelID 
    LEFT JOIN 
        CarType ON Car.carTypeID = CarType.carTypeID 
    LEFT JOIN 
        Status ON Car.statusID = Status.statusID 
    WHERE 
        Employee.userName = p_userName 
    ORDER BY 
        Car.purchaseDate ASC, Status.statusName DESC, Car.description ASC;
END;
$$;
